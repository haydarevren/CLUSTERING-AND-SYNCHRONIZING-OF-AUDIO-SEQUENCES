...
%  MULTIALIGNWRITE, NWALIGN, PROFALIGN, SEQALIGNVIEWER, SEQCONSENSUS,
...
elseif isstruct(iseqs) && isfield(iseqs,'sequence')  % other functions also allow sequence
    ...
    warning(message('bioinfo:multialign:StructSeqCapitalization'));
...
numSeqs = numel(seqs);
if numSeqs<3 
    error(message('bioinfo:multialign:InvalidNumberOfSequences'))
end
...
if bioinfoprivate.isnt(strcat(seqs{:}))
    isAminoAcid = false;
    alpha = 'nt';
elseif bioinfoprivate.isaa(strcat(seqs{:}))
    isAminoAcid = true;
    alpha = 'aa';
else
    ...
    for j=1:2:nvarargin-1
        ...
        elseif length(k)>1
            ...
            switch(k)
                ...
                case 4 % verbose
                    dispLog = bioinfoprivate.opttf(pval);
                    if isempty(dispLog)
                ...
                case 9 % interpolte scoring matices 
                    interpolateSM = bioinfoprivate.opttf(pval);
...
if calculateTree 
    ...
    if numSeqs~=numLeaves
        error(message('bioinfo:multialign:InvalidNumberOfSequencesInTree'))
    end
    if seqNamesGiven

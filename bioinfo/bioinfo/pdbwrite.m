...
%
%   PDBWRITE(FILENAME, PDBSTRUCT) writes the contents of PDBSTRUCT to
%   FILENAME, a PDB-formatted file. PDBSTRUCT is a MATLAB structure
%   containing three dimensional (3D) protein structure coordinate data.
%   PDBSTRUCT is created initially using the GETPDB or PDBREAD functions,
%   and can be edited to modify the 3D protein structure data. The
%   coordinate information is stored in the Model field of PDBSTRUCT. In the
%   output file, the atom serial numbers are preserved. The atomic
%   coordinate records are ordered according to their atom serial numbers.
%
%   Example:
%
%       % Get the Green Fluorescent protein PDB data.
%       gflstruct = getpdb('1GFL')
%
%       % Find the first atom's X coordinate.
%       gflstruct.Model.Atom(1).X
%
%       % Edit the first atom's X coordinate
%       gflstruct.Model.Atom(1).X = -18;
%
%       %Write the modified gflstruct to a PDB-formatted file.
%       pdbwrite('1gfl_out.pdb', gflstruct)
%
%   See also GENPEPTREAD, GETPDB, MOLVIEWER, PDBDISTPLOT, PDBREAD.
...
% Validate input data
if nargin < 1
    error(message('bioinfo:pdbwrite:NotEnoughInputs', mfilename));
end

% If only one input argument we expect a pdbstruct. In this 
% case we output a pdbfile in string form.
if nargin == 1
...
elseif nargin == 2
    ...
    if ~ischar(filename) || isempty(filename),
        error(message('bioinfo:pdbwrite:InvalidFileName'));
    end
    
    % Check pdbstruct is a structure.
    if ~isstruct(pdbstruct)
        error(message('bioinfo:pdbwrite:InvalidInputStructure'))
    end
end

% Check if it is an old PDB structure
if isOldPDBStruct(pdbstruct)
    error(message('bioinfo:pdbwrite:OldPDBStructureDetected'));
end
...
m = isfield(pdbstruct, MandatoryFields);
...
pdbrecord(:,end + 1) = sprintf('\n');
pdbrecord = sprintf('%c', pdbrecord');

if nargout == 1
    varargout{1} = pdbrecord;
end    
...
if ~isempty(filename)
    ...
    if fid == -1
        ...
        if ~isempty(theDir)
            ...
            error(message('bioinfo:pdbwrite:CouldNotOpenFileinCurrentDir', filename));
        end
    end
    fprintf(fid, '%s', pdbrecord);
...
%---------------------------------------------------------%
function isoldstruct = isOldPDBStruct(oldpdbstruct)

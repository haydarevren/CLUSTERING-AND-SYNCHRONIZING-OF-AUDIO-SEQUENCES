...
if nargin > 1
    if nargin < 3
        error(message('bioinfo:bowtie:IncorrectNumberOfArguments'));
    end
...
try   
    ...
    if strncmp(e.message, 'Info:', 5)
        disp(e.message(6:end));
    else
        throw(e);
    end
end
    ...
    switch(arg_index)
        case 1  % BowtieOptions
         ...
         case 2 % BAMFileOutput
            BAMFileOutput = bioinfoprivate.opttf(parameter_value);
         case 3 % Paired
            pairedInput = bioinfoprivate.opttf(parameter_value);
    end
end
end

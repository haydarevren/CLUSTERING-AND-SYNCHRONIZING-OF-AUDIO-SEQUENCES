function h = biograph(cm,cm2,ids,varargin)
...
%
%   BG = BIOGRAPH(CM,IDs) sets the node IDs. IDs is a cell string with the
%   same number of strings as the number of row/columns in CM (nodes). IDs
%   may also be a char array with the same number of rows as nodes or a
%   string with the same number of characters as nodes. IDs must be unique.
%
...
%       get(bg2.nodes,'ID')
%
%   See also BIOGRAPHDEMO, BIOGRAPH.BIOGRAPH/DOLAYOUT,
...
% creates a random graph for exemplification
if nargin == 0
    cm = 15;
end
if nargin == 0 || numel(cm)==1
    n = cm;
    cm = rand(n)<1/8;
    cm = cm - diag(diag(cm));
else
...
if ~setDefaultIDS
    % checking there are not invalid chars (ie control characters)
...
% creating default IDS when appropriate
if setDefaultIDS
    clear ids 
    for k = 1:n
...
% creates the biograph object
h = biograph.biograph(cm,cm2,ids,varargin);

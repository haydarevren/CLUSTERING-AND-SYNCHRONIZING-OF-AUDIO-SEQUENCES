            ...
            switch(k)
                ...
                case 5 %partialseq
                    if iscell(pval) || ischar(pval)
                        error(message('bioinfo:getncbidata:PartialseqWrongTypes'));
                    elseif numel(pval)==1
                ...
                case {6,7}  % 'preambletext'
                    ...
                    passThroughArgs{end+1} = bioinfoprivate.opttf(pval); %#ok<AGROW>
            end
        end
    end
end
...
if isempty(giID)
    ...
    if isempty(giID)
        error(message('bioinfo:getncbidata:NoItemsFound', accessnum, db)) ;
    end
end
...
if fid == (-1)
    error(message('bioinfo:getncbidata:CouldNotOpenFileForRead', urlwritefilename));
end
genbankdata = fscanf(fid,'%c',16384);
fclose(fid);
if ~isempty(regexp(genbankdata,'Error: download dataset is empty','match'))
    error(message('bioinfo:getncbidata:RecordDiscontinuedEmpty', accessnum, db));
end
...
if (tofile == true) && dataIsInTemp
    ...
    if fid1 == (-1)
        error(message('bioinfo:getncbidata:CouldNotOpenFileForWrite', filename));
    end
...
if ~strcmpi(db,type_i)
    ...
    error(message('bioinfo:getncbidata:IncorrectKnownDatabaseSuggest', accessnum, db, 'Nucleotide', program, accessnum));
end
...
elseif strcmpi(recordStatus.status,'dead')
    ...
    warning(message('bioinfo:getncbidata:RecordDiscontinued', accessnum));
elseif strcmpi(recordStatus.status,'suppressed')
    %Warn if record has been suppressed
    warning(message('bioinfo:getncbidata:RecordSuppressed', accessnum, accessnum));
...
elseif strcmpi(recordStatus.status,'replaced')
    ...
    warning(message('bioinfo:getncbidata:RecordReplaced', accessnum, recordStatus.replaced, db, accessnum));
elseif ~strcmpi(recordStatus.accnum,accessnum)
    ...
    warning(message('bioinfo:getncbidata:RecordAutoReplaced', accessnum, recordStatus.accnum, char( giID{ 1 } )));
end
